...
from agent.prompt_builder import build_skills_system_prompt, build_context_files_prompt, build_environment_hints, load_soul_md, load_active_followups_md, TOOL_USE_ENFORCEMENT_GUIDANCE, TOOL_USE_ENFORCEMENT_MODELS, DEVELOPER_ROLE_MODELS, GOOGLE_MODEL_OPERATIONAL_GUIDANCE, OPENAI_MODEL_EXECUTION_GUIDANCE
...
class AIAgent:
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        # Optional follow-up board from HERMES_HOME/active_followups.md
        # Keeps cross-session operational reminders visible to the agent.
        _followups_block = load_active_followups_md()
        if _followups_block:
            prompt_parts.append(_followups_block)
                            ...
                            if response is None:
                                ...
                                if _codex_resp_status in {"failed", "cancelled"}:
                                    ...
                                    if _out_text_stripped:
                                        ...
                                        # Prior turn delivered the final answer alongside tool calls;
                                        # an empty follow-up is expected. Synthesize a minimal
                                        # output so _normalize_codex_response doesn't raise.
                                        logger.debug(
                                        ...
                                        # Streaming delivered text deltas but get_final_response()
                                        # returned empty output. Synthesize from captured deltas.
                                        ...
                                        # Post-tool-execution empty response: model processed
                                        # tool results and has nothing more to say. Normal —
                                        # synthesize a minimal output so the agent loop exits
                                        # cleanly via the "No tool calls" fallback path.
                                        logger.debug(
