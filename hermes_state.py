    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 7:
                # v7 (upstream): preserve provider-native reasoning_content separately
                # from normalized reasoning text. Kimi/Moonshot replay can require
            ...
            if current_version < 8:
                # v8 (upstream): add api_call_count column to sessions — tracks the
                # number of individual LLM API calls made within a session (as
                # opposed to the session count itself).
                try:
                    cursor.execute(
            ...
            if current_version < 9:
                # v9 (fork): add unified_timeline table + FTS + lookup index.
