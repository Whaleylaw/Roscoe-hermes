...
class TestDelegateHeartbeat(unittest.TestCase):
    ...
    def test_heartbeat_does_not_trip_idle_stale_while_inside_tool(self):
        ...
        # Patch the interval and idle ceiling so the test proves the
        # in-tool branch takes effect without depending on exact scheduler
        # cadence. If a busy child incorrectly used the idle limit, the loop
        # would stop before the third touch; the in-tool limit should keep
        # emitting heartbeats until the child completes.
        with patch("tools.delegate_tool._HEARTBEAT_INTERVAL", 0.05), \
